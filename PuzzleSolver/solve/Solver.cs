...
namespace PuzzleSolver
{
    internal class Solver
    {
        ...
        public Board solveAStar(Board state, int type)
        {
            ...
            int totalState = state.getStateDepth() + state.getManhattanDistance();// O(N^2)
            state.setTotalDest(totalState);
            openList.enqueue(state);// O(1)
            Board currentState = null;
            //int c = 0;
            while (openList.count() > 0)// O(E)
            {
                currentState = openList.dequeue();// O(Log(v)) 
                currentState.gridToHash(); // O(N^2)
                closeList.Add(currentState.hashCode); // O(1)
                //c++;
                if (currentState.getTotalDest() == currentState.getStateDepth())//O(1)
                {
                    break;
                }
                currentState.neighboors(ref openList, ref closeList, type);//O(N^2)
            }
            //Console.WriteLine(c);
